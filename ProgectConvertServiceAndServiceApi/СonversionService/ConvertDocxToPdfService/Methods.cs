...
namespace ConvertDocxToPdfService
{
    class Methods:IMethods
    {
        public async Task QueueLiquidatorAsync() //a method that records completed tasks in the database
        {
            await Task.Run(async () =>
            {
                using (var db = new MyDbContext())
                {
                    ...
                    {
                        await Task.Delay(50);
                        if (StartService.queueTaskId.Count() != 0)
                        {
                            int taskId = StartService.queueTaskId.Dequeue();
        ...
        public Reserv SelectClient(Queue<Reserv>[] nameArrayQueues) //selection algorithm from queues with different priorities.
        {
            ...
            do
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    {
                        queueWeight[i] = DateTime.Now.Subtract(nameArrayQueues[i].Peek().TimeRegistrInDb).TotalMilliseconds * StartService.priorityRatio[i];
                    }
                    else
                    { queueWeight[i] = 0.0; }
                }
        ...
        public void Convert(Reserv res) //convert doc to pdf. Using package sautinsoft.socument.
        {
            ...
            StartService.queueTaskId.Enqueue(res.TaskId);
        }
